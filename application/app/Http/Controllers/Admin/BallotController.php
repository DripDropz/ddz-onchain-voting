...
class BallotController extends Controller {
    ...
    public function index() {
        ...
        return Inertia::render('Auth/Ballot/Index', [
                'ballots' => $ballots,
                'crumbs'  => $crumbs,
            ]);
    }
    ...
    public function store(BallotData $ballotData): RedirectResponse {
        ...
        if ($response->allowed()) {
            ...
            return Redirect::back()
                           ->withErrors(['error' => 'Not authorized to create ballot']);
        }
    }
    ...
    public function update(BallotData $ballotData) {
        ...
        $response = $ballotData->status == 'published' ? Gate::inspect('publish', $ballot) : Gate::inspect('update',
            $ballot);
        ...
        if ($response->allowed()) {
            ...
            return Redirect::back()
                           ->withErrors(['error' => 'Not authorized']);
        }
    }
    ...
    public function destroy(Request $request, Ballot $ballot): RedirectResponse {
        ...
        if (Hash::check($password, Auth::user()->password)) {
            ...
            return Redirect::back()
                           ->withErrors(['password' => 'Invalid password for current user.']);
        }
    }
    ...
    public function statusUpdate(Ballot $ballot) {
        ...
        $response = $ballot->status == 'published' ? Gate::inspect('publish', $ballot) : Gate::inspect('update',
            $ballot);
        ...
        if ($response->allowed()) {
            $crumbs = [
                [
                    ...
                    'link'  => route('admin.ballots.questions.create', ['ballot' => $ballot->hash]),
                ],
            ];
            ...
            return Redirect::back()
                           ->withErrors(['error' => 'Not authorized to create question']);
        }
    }
    ...
    public function editQuestion(Request $request, Ballot $ballot, Question $question) {
        ...
        $crumbs = [
            [
                'label' => 'Edit Question',
                'link'  => route('admin.ballots.questions.edit', ['ballot' => $ballot->hash]),
            ],
        ];
    ...
    public function updateQuestion(Request $request, $ballot, $question): RedirectResponse {
        ...
        if ($response->allowed()) {
            ...
            return redirect()
                ->route('admin.ballots.view', ['ballot' => $ballot->hash])
                ->withErrors([
    ...
    public function destroyQuestion(Request $request, Ballot $ballot, Question $question) {
        ...
        if ($response->allowed()) {
            $queestion = Question::where('id', $question->id)
                                 ->first();
            $queestion->choices()
                      ->delete();
            $queestion->delete();
            ...
            return Redirect::back()
                           ->withErrors(['error' => 'Not authorized to delete question']);
        }
    }
    ...
    public function storePolicy(Request $request, Ballot $ballot) {
        ...
        // create wallet that will be the signer on policy
        $wallet = new Wallet(['passphrase' => $data['seedphrase']]);
        ...
        $getPolicyIdRequest->body()
                           ->merge([
                               'seed' => $policy?->wallet?->passphrase,
                           ]);
        $response          = $connector->sendAndRetry($getPolicyIdRequest, 2, 300,
